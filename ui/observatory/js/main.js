...
/** Docker: UI/API :3000 → sensing WS :3001. Standalone: :8080 → :8765. */
function buildSensingWsUrl(locationLike = window.location) {
  const protocol = locationLike.protocol === 'https:' ? 'wss:' : 'ws:';
  const hostname = locationLike.hostname || 'localhost';
  const port = locationLike.port || '';
  const mapped = { '3000': '3001', '8080': '8765' }[port];
  const host = mapped ? `${hostname}:${mapped}` : (locationLike.host || `${hostname}:3001`);
...
class Observatory {
  ...
  _autoDetectLive() {
    // Prefer live WS. Docker serves UI/API on :3000 and the sensing stream on :3001
    // (see buildSensingWsUrl). Demo is opt-in only via Settings.
    if (this.settings.dataSource !== 'ws') return;
    ...
    const tryNext = (i) => {
      if (i >= unique.length) {
        // Health probe failed — still try the mapped WS URL for this page origin.
        const fallback = this.settings.wsUrl || buildSensingWsUrl();
        console.log('[Observatory] Health probe missed; connecting WS →', fallback);
        this.settings.wsUrl = fallback;
        void this._connectWS(fallback);
        return;
      }
      const base = unique[i];
  ...
  async _connectWS(url) {
    ...
    const target = url || this.settings.wsUrl || buildSensingWsUrl();
    this.settings.wsUrl = target.replace(/\?.*$/, '');
    let wsUrl = target;
    try { wsUrl = await withWsTicket(target); } catch { /* auth off or pre-ADR-272 server */ }
    try {
      ...
      this._ws.onclose = () => {
        console.log('[Observatory] WebSocket closed; reconnecting (live mode)');
        this._ws = null;
        this._hud.updateSourceBadge('ws', null);
        if (this.settings.dataSource === 'ws') this._scheduleWsReconnect();
      };
      this._ws.onerror = () => {};
    } catch {}
  }
  ...
  _scheduleWsReconnect() {
    ...
    const attempt = (this._wsReconnectAttempt || 0);
    const delay = Math.min(1000 * (2 ** Math.min(attempt, 4)), 16000);
    this._wsReconnectAttempt = attempt + 1;
    this._wsReconnectTimer = setTimeout(() => {
      this._wsReconnectTimer = null;
      if (this.settings.dataSource === 'ws') {
        void this._connectWS(this.settings.wsUrl || buildSensingWsUrl());
      }
    }, delay);
  }

  _disconnectWS(clearLive = true) {
    ...
    if (clearLive) this._liveData = null;
  }
  ...
  _animate() {
    ...
    // Data source — live mode never feeds the demo generator
    if (this.settings.dataSource === 'ws') {
      if (this._liveData) this._currentData = this._liveData;
    } else {
