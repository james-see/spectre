...
export default class TrainingPanel {
  constructor(container) {
    ...
    this.config = {
      ...
      // Session tags for supervised / room adaptation runs
      recLabel: 'walk',
      recName: '',
    };
    this._labelPresets = [
      'empty', 'stand', 'walk', 'sit', 'lie', 'gesture', 'multi', 'pose',
    ];
  ...
  async _startRec() {
    this._set({ loading: true, error: null });
    try {
      const label = (this.config.recLabel || 'pose').trim() || 'pose';
      const session_name = (this.config.recName || '').trim()
        || `${label}_${Date.now()}`;
      await trainingService.startRecording({ session_name, label });
  ...
  _renderRecordings() {
    ...
    // Label / name controls (API supports label; stock UI previously hard-coded "pose")
    if (!this.state.isRecording) {
      const form = this._el('div', 'tp-config-form');
      form.style.marginBottom = '8px';

      const nameRow = this._el('div', 'tp-input-row');
      nameRow.appendChild(this._el('label', 'tp-label', 'Session name'));
      const nameInp = document.createElement('input');
      nameInp.type = 'text';
      nameInp.className = 'tp-input';
      nameInp.style.width = '160px';
      nameInp.placeholder = 'auto from label';
      nameInp.value = this.config.recName || '';
      nameInp.addEventListener('change', () => { this.config.recName = nameInp.value; });
      nameInp.addEventListener('input', () => { this.config.recName = nameInp.value; });
      nameRow.appendChild(nameInp);
      form.appendChild(nameRow);

      const labelRow = this._el('div', 'tp-input-row');
      labelRow.appendChild(this._el('label', 'tp-label', 'Activity label'));
      const sel = document.createElement('select');
      sel.className = 'tp-input';
      sel.style.width = '160px';
      this._labelPresets.forEach((lab) => {
        const opt = document.createElement('option');
        opt.value = lab;
        opt.textContent = lab;
        if (lab === this.config.recLabel) opt.selected = true;
        sel.appendChild(opt);
      });
      const custom = document.createElement('option');
      custom.value = '__custom__';
      custom.textContent = 'custom…';
      if (this.config.recLabel && !this._labelPresets.includes(this.config.recLabel)) {
        custom.selected = true;
      }
      sel.appendChild(custom);
      sel.addEventListener('change', () => {
        if (sel.value === '__custom__') {
          const v = prompt('Custom activity label:', this.config.recLabel || '');
          if (v && v.trim()) this.config.recLabel = v.trim();
          else sel.value = this.config.recLabel || 'walk';
        } else {
          this.config.recLabel = sel.value;
        }
        this.render();
      });
      ...
      if (this.config.recLabel && !this._labelPresets.includes(this.config.recLabel)) {
        form.appendChild(this._el('div', 'tp-rec-meta', `custom label: ${this.config.recLabel}`));
      }
      s.appendChild(form);
    } else {
      s.appendChild(this._el(
        'div',
        'tp-rec-meta',
        `Recording… label=${this.config.recLabel || 'pose'}`,
      ));
    }
    ...
    if (this.state.recordings.length === 0 && !this.state.loading) {
      ...
      this.state.recordings.forEach(rec => {
        ...
        const parts = [];
        if (rec.label) parts.push('label=' + rec.label);
        const frames = rec.frame_count ?? rec.frames;
        if (frames != null) parts.push(frames + ' frames');
        const bytes = rec.file_size_bytes ?? rec.size_bytes;
        if (bytes != null) parts.push(this._fmtB(bytes));
    ...
    if (this.state.isRecording) {
      ...
      const b = this._btn(
        `Start Recording (${this.config.recLabel || 'pose'})`,
        'tp-btn tp-btn-rec',
        () => this._startRec(),
      );
      b.disabled = this.state.loading; acts.appendChild(b);
